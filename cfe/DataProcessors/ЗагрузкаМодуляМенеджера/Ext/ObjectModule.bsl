...
Перем КД3_Fix_НеНайденныеОбъекты;

&ИзменениеИКонтроль("ЗагрузитьПКО")
Процедура КД3_Fix_ЗагрузитьПКО(СтрокаРезультат, Конфигурация, СчетчикПКО, СуществующиеПКО, СтрокаЗаполнитьПКО, ТаблицаРезультатов, РодственныеКонвертации, РодственныеКонфигурации)
...
#Вставка
			Если НЕ ПустаяСтрока(ИмяОбъектаКонфигурации)
			   И НЕ ЗначениеЗаполнено(ПКООбъект.ОбъектКонфигурации)
			   И НЕ ИмяОбъектаКонфигурации = "Неопределено" Тогда
				Если КД3_Fix_НеНайденныеОбъекты.Найти(ИмяОбъектаКонфигурации) = Неопределено Тогда
					КД3_Fix_НеНайденныеОбъекты.Добавить(ИмяОбъектаКонфигурации);
				КонецЕсли;
			КонецЕсли;
#КонецВставки
...
#КонецВставки
	...
	Если СтрокаРезультат.Отправка = Ложь И СтрокаРезультат.Получение = Ложь Тогда
		ОпределитьНаправлениеОбменаПКО(СтрокаЗаполнитьПКО, ТаблицаРезультатов);
	КонецЕсли;
...
&Вместо("ЗагрузитьПроцедурыИФункцииВОбъектыМетаданных")
Процедура КД3_Fix_ЗагрузитьПроцедурыИФункцииВОбъектыМетаданных(ТаблицаРезультатов)
	
	КД3_Fix_НеНайденныеОбъекты = Новый Массив;
	
	ПродолжитьВызов(ТаблицаРезультатов);
	
	Если КД3_Fix_НеНайденныеОбъекты.Количество() > 0 Тогда
		Если КД3_Fix_НеНайденныеОбъекты.Количество() = 1 Тогда
			ТекстСообщения = "Не найден объект метаданных:";
		Иначе
			ТекстСообщения = "Не найдены объекты метаданных:";
		КонецЕсли;
		Для Каждого ИмяОбъектаМетаданных Из КД3_Fix_НеНайденныеОбъекты Цикл
			ТекстСообщения = ТекстСообщения + Символы.ПС + Символы.Таб + СтрЗаменить(ИмяОбъектаМетаданных, "Метаданные.", "");
		КонецЦикла;
